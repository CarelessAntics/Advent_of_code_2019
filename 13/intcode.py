...
def drawScreen(screen):
	screen2 = {i: j for i, j in screen.items() if type(i) != str}
	#print(screen2)
	screenSize = (max(screen2.keys(), key=lambda l: l[0])[0], max(screen2.keys(), key=lambda l: l[1])[1])
	#print(screenSize)
	for y in range(screenSize[1]):
		messageLine = []
		for x in range(screenSize[0]+1):
			if   screen[x,y] == 0: messageLine.append(' ')
			elif screen[x,y] == 1: messageLine.append('#')
			elif screen[x,y] == 2: messageLine.append('+')
			elif screen[x,y] == 3: messageLine.append('=')
			elif screen[x,y] == 4: messageLine.append('O')
			else: messageLine.append('?')
		print(''.join(messageLine))
	return
...
def runGame(data):
	...
	outputTracker = 0
	x, y = 0, 0
	paddlePos, ballPos = 0, 0
	screen = {"score":0, (0,0): 0}

	while output[2] != 'stopped':

		if 3 in screen.values() and 4 in screen.values():
				paddlePos = list(screen.keys())[list(screen.values()).index(3)]
				ballPos = list(screen.keys())[list(screen.values()).index(4)]
				if   paddlePos[0] < ballPos[0]: uInput = 1
				elif paddlePos[0] > ballPos[0]: uInput = -1
				else: uInput = 0
		...
		elif outputTracker == 1:
			y = output[1]
			outputTracker += 1

		elif outputTracker == 2:
			if (x, y) == (-1, 0):
				screen["score"] = output[1]
			else:
				screen[x,y] = output[1]
			#print(screen)
			drawScreen(screen)
